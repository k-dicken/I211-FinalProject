...
window.onload = function () {
    ...
    //DOM objects
    fromSearchBoxObj = document.getElementById('from');
    // toSearchBoxObj = document.getElementById('to');
    fromSuggestionBoxObj = document.getElementByClassName('from-search');
    // toSuggestionBoxObj = document.getElementByClassName('to-search');
};

window.onclick = function () {
    fromSuggestionBoxObj.style.display = 'none';
    // toSuggestionBoxObj.style.display = 'none';
};
...
function suggest(query) {
    ...
    if (query === "") {
        fromSuggestionBoxObj.innerHTML = "";
        return;
    }
    ...
    xmlHttp.onreadystatechange = function () {
        ...
        if (xmlHttp.readyState === 4 && xmlHttp.status === 200) {
            ...
            var flight = JSON.parse(xmlHttp.responseText);
            //console.log(titlesJSON);
            // display suggested titles in a div block
            displayTitles(flight);
        }
    };
